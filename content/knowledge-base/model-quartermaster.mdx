...
> **Note:** The `cortex qm` and `cortex mqm` CLI commands documented below are available via the Web UI at `http://localhost:3000` (Quartermaster tab). These CLI commands currently exist as separate modules but are not registered in the main CLI command tree. Access Quartermaster data through the web dashboard for the most up-to-date interface.
---
