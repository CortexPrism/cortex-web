...
| Option | Description |
|--------|-------------|
| `--type` | Filter results by memory type: `episodic`, `semantic`, or `all` (default: `all`) |
...
|------|-------------|---------|
| Ephemeral (T1) | Session | Current conversation context window |
| Episodic (T2) | Session | Turn summaries of user+agent exchanges |
| Semantic (T3) | Long-term | Injected facts and knowledge |
| Archival (T4) | Permanent | Compressed historical data, entities |
| Reflection (T5) | Permanent | LLM-extracted behavior patterns, per-turn meta-cognition |
