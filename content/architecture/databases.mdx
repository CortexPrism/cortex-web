...
| Session | `sessions/<id>.db` | Per-session message history | 006 |

## Migration History

Core migrations (37 total across all databases):
...
| 006_session.sql | sessions/<id>.db | Per-session message store |
...
| 010-037 | Various | Hub nodes, agents, services, skills, channels, workflows, onboarding, evaluations, triggers, MQM, voice, computer use, sandboxes, memori checkpoints, supply chain verification, compliance, and more |
...
│   ├── vault.db
│   ├── plugins.db
│   └── sessions/<id>.db  (one per chat session)
```
